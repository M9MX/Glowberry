...
import org.m9mx.cactus.glowberry.util.appleskin.api.event.FoodValuesEvent;
...
public class FoodHelper
{
	...
	public static FoodProperties modifyFoodProperties(Player player, FoodProperties defaultFood, Consumable consumable, ItemStack itemStack)
	{
		// Create a FoodProperties based on the default one but potentially modified by events
		FoodValuesEvent foodValuesEvent = new FoodValuesEvent(player, itemStack, defaultFood, defaultFood);
		FoodValuesEvent.EVENT.invoker().interact(foodValuesEvent);

		return foodValuesEvent.modifiedFoodComponent;
	}
	...
	public static float getEstimatedHealthIncrement(Player player, ConsumableFood consumableFood)
	{
		...
		for (var effect : consumableFood.consumable().onConsumeEffects())
		{
			...
			for (var statusEffect : ((ApplyStatusEffectsConsumeEffect) effect).effects())
			{
				if (statusEffect.getEffect().is(net.minecraft.world.effect.MobEffects.REGENERATION))
				{
	...
	public static float getEstimatedHealthIncrement(int foodLevel, float saturationLevel, float exhaustionLevel)
	{
		...
		while (foodLevel >= 18)
		{
			...
			if (foodLevel >= 20 && Float.compare(saturationLevel, Float.MIN_NORMAL) > 0)
			{
				...
				// in a loop can end up with a different (and higher) result than multiplication
